...
class NotificationQueue {
  ...
  async sendNotification(item) {
    ...
    if (fcmTokens.length === 0) {
      console.warn(`⚠️ No FCM tokens found for notification: ${title}`)
      // Still save to database even if no FCM tokens
    }

    // Send via Firebase (only if tokens exist)
    let result = null
    if (fcmTokens.length > 0) {
    ...
    // Save notification to database for each user AFTER FCM send
    ...
    try {
      ...
      console.log(`✅ Saved ${userIds.length} notification(s) to database`)
    } catch (dbError) {
