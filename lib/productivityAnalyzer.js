...
export async function analyzeProductivityData(data) {
  ...
  if (GEMINI_API_KEY) {
    try {
      return await analyzeWithGemini(context, data);
    } catch (error) {
 ...
 */
async function analyzeWithGemini(context, data) {
  const response = await fetch(
    `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${GEMINI_API_KEY}`,
    {
      ...
      body: JSON.stringify({
        contents: [{
          parts: [{
            text: `You are a productivity analyst AI. Analyze this employee's computer usage data and provide helpful insights.
