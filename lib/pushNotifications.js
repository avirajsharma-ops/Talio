...
export async function sendPushNotification({
  ...
  try {
    ...
    if (!token) {
      console.warn('No admin token provided for push notification')
      return { success: false, message: 'No admin token' }
    }

    const response = await fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/api/push-notifications/send`, {
      method: 'POST',
      headers: {
