...
export async function processAutoCheckouts() {
  try {
    ...
    // Auto-checkout time is 30 mins after scheduled checkout
    const autoCheckoutTriggerTime = new Date(scheduledCheckoutTime.getTime() + 30 * 60 * 1000)
    
    // Only proceed if we're past the auto-checkout trigger time
    if (now < autoCheckoutTriggerTime) {
      return { processed: 0, reason: 'Not yet time for auto-checkout' }
    }
    ...
    for (const attendance of stillCheckedIn) {
      try {
        // Auto-checkout at SCHEDULED checkout time (NOT including the 30 min buffer)
        // This ensures the extra 30 mins are NOT counted
        ...
        attendance.remarks = (attendance.remarks || '') + ' | Auto-checked out 30 mins after scheduled time. Extra time not counted. Raise correction request for overtime.'
        ...
        if (attendance.employee?.user) {
          ...
          await sendPushToUser(
            attendance.employee.user._id,
            {
              ...
              body: `${message} You were checked out at ${scheduledCheckoutTime.toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}. If you worked overtime, please raise a correction request.`,
            },
            {
              ...
              data: {
                ...
                note: 'Extra time after scheduled checkout not counted. Raise correction request for overtime.',
              },
            }
          )
        }
        ...
        processedCount++
        console.log(`[AttendanceScheduler] Auto-checkout processed for ${attendance.employee?.email || attendance._id} at scheduled time ${scheduledCheckoutTime.toLocaleTimeString()}`)
      } catch (err) {
