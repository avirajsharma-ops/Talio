 ...
 *
 * DATA ACCESS RULES:
 * - GOD ADMIN: Access to every minute detail, no restrictions
 * - ADMIN/HR: Access to all employee information and data
 * - DEPARTMENT HEAD: Access to all employees in their department
 * - MANAGER: Access to their direct reports
 * - EMPLOYEE: Access to their own data and linked task data only
 *
 * MESSAGING RULES:
 * - No hierarchy required - anyone can message anyone through MAYA
 *
 * SCREEN MONITORING RULES:
 * - ADMIN: Can monitor everyone
 * - DEPARTMENT HEAD: Can monitor employees in their department
 * - GOD ADMIN: Can monitor anyone anywhere
 * - Others: No screen monitoring access
 */
...
const COLLECTION_PERMISSIONS = {
  ...
  // Department Head - FULL access to department employees and their data
  department_head: {
    read: ['employees', 'departments', 'attendance', 'leave', 'performance', 'tasks', 'projects', 'announcements', 'documents', 'assets', 'activities', 'timesheets', 'expenses', 'travel', 'goals', 'training', 'feedback', 'warnings', 'appreciations'],
    create: ['tasks', 'projects', 'announcements', 'performance', 'goals', 'training'],
    update: ['employees', 'attendance', 'leave', 'tasks', 'projects', 'performance', 'goals'],
    delete: [],
  },
 ...
 */
export function canAccessEmployeeData(userRole, requestingUserId, targetEmployeeId, employeeData, requestingUserDepartmentId = null) {
  ...
  if (userRole === 'department_head' && requestingUserDepartmentId) {
    if (employeeData && employeeData.department?.toString() === requestingUserDepartmentId?.toString()) {
      return { allowed: true, reason: 'department_head_access' };
    }
  }
...
export function getAllowedFields(userRole, collection, action) {
  ...
  const SENSITIVE_FIELDS_TO_HIDE = {
    employees: {
      ...
      department_head: [], // Department heads can see ALL employee data in their department including salary, bank details
    ...
    payroll: {
      ...
      department_head: [], // Department heads CAN see payroll details of their department employees
 ...
 *
 * DATA ACCESS RULES:
 * - GOD ADMIN: See everything, no filters
 * - ADMIN/HR: See all employee information
 * - DEPARTMENT HEAD: See employees in their department
 * - MANAGER: See their direct reports
 * - EMPLOYEE: See only their own data and linked task data
 */
export function buildRoleBasedFilter(userRole, collection, userId, employeeId, departmentId = null) {
  const filters = {};
  ...
  // Department head sees their department - FULL ACCESS to all department data
  if (userRole === 'department_head' && departmentId) {
    if (collection === 'employees') {
      filters.department = departmentId;
      return filters;
    }
    // For documents - show department documents and documents uploaded by department employees
    ...
    // For activities - show activities of department employees
    if (collection === 'activities') {
      // Will be filtered by department employees in the API
      return filters;
    }
    // For tasks - show tasks assigned to or created by department employees
    if (collection === 'tasks') {
      // Will be filtered by department employees in the API
      return filters;
    }
    // For employee-specific collections, filter by department employees
    if (['attendance', 'leave', 'expenses', 'travel', 'payroll', 'performance', 'timesheets', 'goals', 'training', 'feedback', 'warnings', 'appreciations'].includes(collection)) {
      // Will be filtered by department employees in the API
      return filters;
    }
  }
  ...
  if (employeeCollections.includes(collection)) {
    if (userRole === 'employee') {
      ...
      // This would need to be enhanced with actual team logic
      return filters;
    }
  }

  // For tasks, employees can see tasks they're linked to
  if (collection === 'tasks' && userRole === 'employee') {
    filters.$or = [
...
/**
 * Check if user can monitor another user's screen
 */
export function canMonitorScreen(monitoringUserRole, monitoringDepartmentId, targetDepartmentId) {
  // GOD ADMIN can monitor anyone anywhere
  if (monitoringUserRole === 'god_admin') {
    return { allowed: true, reason: 'GOD ADMIN has unrestricted monitoring access' };
  }

  // ADMIN can monitor everyone
  if (monitoringUserRole === 'admin') {
    return { allowed: true, reason: 'Admin can monitor all employees' };
  }

  // Department head can monitor employees in their department
  if (monitoringUserRole === 'department_head') {
    if (monitoringDepartmentId && targetDepartmentId &&
        monitoringDepartmentId.toString() === targetDepartmentId.toString()) {
      return { allowed: true, reason: 'Department head can monitor employees in their department' };
    }
    return { allowed: false, reason: 'Department head can only monitor employees in their own department' };
  }

  // Others cannot monitor
  return { allowed: false, reason: 'You do not have screen monitoring permissions' };
}
