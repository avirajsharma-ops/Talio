...
// Screenshot interval: 1 per minute
// Expected screenshots per session: 30
const SESSION_DURATION_MINS = 30;
const SCREENSHOT_INTERVAL_MINS = 1;
const EXPECTED_SCREENSHOTS_PER_SESSION = 30;
...
/**
 * Get session interval - HARDCODED to 30 minutes
 * Settings are read but interval is always 30 mins
 */
async function getSessionInterval() {
  // Always return 30 minutes - hardcoded
 ...
 * IMPORTANT: Preserves existing sessions with AI analysis - only updates if no analysis exists
 */
export async function saveAggregatedSessions(sessions) {
  ...
  for (const session of sessions) {
    try {
      ...
      if (existing) {
        // Check if existing session has AI analysis - if so, skip update to preserve it
        const hasExistingAnalysis = existing.aiAnalysis && 
          existing.aiAnalysis.analyzedAt && 
          (existing.aiAnalysis.summary || existing.aiAnalysis.productivityScore > 0);
        
        if (hasExistingAnalysis) {
          console.log(`[Session Aggregator] Skipping session ${existing._id} - already has AI analysis`);
          savedSessions.push(existing);
          continue;
        }
        
        // Only update if no analysis exists - merge screenshots and data
        const mergedScreenshots = [...(existing.screenshots || [])];
        const existingScreenshotTimes = new Set(mergedScreenshots.map(s => s.capturedAt?.getTime()));
        
        // Add new screenshots that don't already exist
        for (const screenshot of (session.screenshots || [])) {
          const screenshotTime = screenshot.capturedAt?.getTime();
          if (!existingScreenshotTimes.has(screenshotTime)) {
            mergedScreenshots.push(screenshot);
          }
        }
        
        // Update only non-analyzed sessions
        existing.screenshots = mergedScreenshots;
        existing.appUsageSummary = session.appUsageSummary;
        existing.websiteVisitSummary = session.websiteVisitSummary;
        existing.keystrokeSummary = session.keystrokeSummary;
        existing.mouseActivitySummary = session.mouseActivitySummary;
        existing.totalActiveTime = session.totalActiveTime;
        existing.productiveTime = session.productiveTime;
        existing.neutralTime = session.neutralTime;
        existing.unproductiveTime = session.unproductiveTime;
        existing.topApps = session.topApps;
        existing.topWebsites = session.topWebsites;
        existing.captureCount = session.captureCount;
        existing.sourceDataIds = [...new Set([...(existing.sourceDataIds || []), ...(session.sourceDataIds || [])])];
        
        await existing.save();
        savedSessions.push(existing);
        console.log(`[Session Aggregator] Updated session ${existing._id} (no existing AI analysis)`);
      } else {
        // Create new session
